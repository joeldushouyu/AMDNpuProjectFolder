...
void matvec_vectorized_int8(T *__restrict a, T *__restrict b, T_out *__restrict c) {
    ...
    chess_prepare_for_pipelining chess_loop_range(2, ){
        ...
        chess_flatten_loop {
            aie::vector<T, 16> b_vec = aie::load_v<16>(b_ptr); // 16 int8 data 
            //TODO: doesn't the manual say it affect if register split?
            //TODO: b is rearrange into column order in memory?
            const aie::vector<T, 4 * r> a_vec_0 = aie::load_v<4 * r>(a_ptr); // 4*16* *(int8) = 512 bit, 
            ...
            // const aie::vector<T, r> a_vec_12_1 = aie::T_outfilter_odd(a_vec_12, 1);
