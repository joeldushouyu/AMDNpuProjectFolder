...
int main(int argc, const char *argv[]) {
    ...
    // Add custom options
    desc.add_options()("M,m", po::value<int>()->default_value(64), "matrixRow_COL_dimension"); // don't change it yet

    arg_utils::parse_options(argc, argv, desc, vm);
    
    // User logic
    int rowColNumber = vm["M"].as<int>();
    int matrixSize = rowColNumber*rowColNumber;
    ...
    for(auto row = 0; row < rowColNumber; row++){
        for(auto col = 0; col < rowColNumber; col++){
            w_0[ col + row*rowColNumber] = col;
            y_ref[ col + row*rowColNumber] = col + 12;
        }
    }
