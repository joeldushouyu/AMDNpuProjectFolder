...
//    }
    ...
    OUT_SIZE = 264; //want to include both CT->MT header and MT->SHimtile header
  ...
  // setup package header, note in lsb
  if (app_id == 0){
    srcVecA[0] = 0;
    srcVecA[1] = 0; // the kernel look at 11-8th bit(LSB) to determine if multiple/add
    srcVecA[2] = 0; // doesnot matter
    srcVecA[3] = 0;
  }else if(app_id == 1){
    srcVecA[0] = 0;
    srcVecA[1] = 1 ; // the kernel look at 11-8th bit(LSB) to determine if multiple/add
    srcVecA[2] = 0; // doesnot matter
    srcVecA[3] = 0;
  }else{
    // nothing;
  ...
  std::cout << "Header 0(MT->ShimT): 0x" << std::hex << std::uppercase << packet_header_receive_1 << std::endl;
