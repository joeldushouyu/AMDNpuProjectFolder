    ...
    def device_body():
        ...
        def m(block):
            ...
            with block[4]:
                ...
                dma_bd(inA_mem_buffers[0], offset=0, len=m_x_k, dimensions=[  (k_size // t_size, t_size*k_size   ), (k_size, 1), (t_size, k_size)]   )  
                use_lock(inA_mem_prod_lock, LockAction.Release, value=1)
                next_bd(block[5])
            with block[5]:
                use_lock(inA_mem_con_lock, LockAction.AcquireGreaterEqual, value=1)
                dma_bd(inA_mem_buffers[1], offset=0, len=m_x_k, dimensions=[  (k_size // t_size, t_size*k_size   ), (k_size, 1), (t_size, k_size)]   )
                use_lock(inA_mem_prod_lock, LockAction.Release, value=1)
                next_bd(block[4])
